	...
	if (empty($servername)) {
		...
		if(file_exists($db_file)){
			...
			$tbk_zjy = "CREATE TABLE tbk_zjy (
				...
				zdy_text TEXT(1000) NULL,
				gdydwa_1 TEXT(1000) NULL,
				gdydwa_2 TEXT(1000) NULL,
					...
					if ($conn->query($tbk_user) === TRUE) {
						...
						if ($conn->query($tbk_set) === TRUE) {
							...
							if ($conn->query($tbk_dwzapi) === TRUE) {
								...
								// 创建默认短网址API
								$dwzapiurl = "http://".$_SERVER['HTTP_HOST'].dirname(dirname($_SERVER["REQUEST_URI"]))."/api/kuaizhan.php?long_url=";
								mysqli_query($conn,"INSERT INTO tbk_dwzapi (dwztitle, dwzapi) VALUES ('快站短网址', '$dwzapiurl')");
