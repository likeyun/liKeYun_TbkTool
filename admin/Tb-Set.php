...
if(isset($_SESSION["tbktools.admin"])){
  ...
  // 获得淘宝用户名
  $sql_set = "SELECT * FROM tbk_set WHERE Set_Obj = 'zjy_tbname'";
  $result_set = $conn->query($sql_set);

  echo '<div class="container">
    <h2 class="big-title">淘宝客工具箱 - 淘宝设置</h2>
    <p class="tips">本面板提供淘宝用户名设置，用户名必须要与授权的一致。</p>
    <ul class="nav nav-pills" role="tablist">
      <li class="nav-item">
        <a class="nav-link active" href="#">淘宝设置</a>
      </li>
      <li class="nav-item">
    ...
    if ($result_set->num_rows > 0) {
      while($row_set = $result_set->fetch_assoc()) {
        // PID
        $tbname = $row_set["Set_Val"];

        echo '<div class="card">
        <div class="card-body">
        <h4 class="card-title">'.$tbname.'</h4>
        <a href="Edi-Set.php?setobj=tbname" class="card-link">编辑</a>
        </div>
        </div>
        <br/>';
      }
